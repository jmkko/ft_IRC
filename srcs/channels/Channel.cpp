...
Channel::Channel(const Channel &other)
    : _name(other._name), _topic(other._topic), _key(other._key), _mode(other._mode), _userLimit(other._userLimit),
      _members(other._members), _invites(other._invites), _operators(other._operators),
...
void Channel::broadcast(Server &server, ReplyCode replyCode, const std::string &params, Client *sender,
...
void Channel::broadcast_bot(Server &server, ReplyCode replyCode, const std::string &params, Client *sender,
                            const std::string &trailing) const {
    ReplyHandler &rh = ReplyHandler::get_instance(&server);
    LOG_DV_CMD(_members.size());
    for (std::set<Client *>::iterator it = _members.begin(); it != _members.end(); ++it) {
