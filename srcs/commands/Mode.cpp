...
static void parse_params(std::string &params,
  ...
  if (args.empty())
    return;
  if (!Utils::is_char_of(args[0], std::string(MODE_OPERATOR)) ||
      args.size() == 1)
    modeQueue->push(args);
  else if (args.size() > 1) {
    if (std::string(args.begin() + 1, args.end())
            .find(std::string(MODE_OPERATOR)) != std::string::npos)
      modeQueue->push(args);
    else
  ...
  while (iss >> args) {
    LOG_DV_CMD(args);
    if (args.size() > 1 &&
        Utils::is_char_of(args[0], std::string(MODE_OPERATOR)) &&
        !std::isdigit(args[1])) {
      if (std::string(args.begin() + 1, args.end())
              .find(std::string(MODE_OPERATOR)) != std::string::npos)
        modeQueue->push(args);
      else
        for (std::string::iterator it = args.begin() + 1; it != args.end();
             ++it)
          modeQueue->push(std::string(1, args[0]) + *it);
    } else
 ...
 *                     🛠️ FUNCTIONS                          *
 *************************************************************/
bool Mode::_simple_args(Server &server, Client &client, Channel *&channel,
                        Parser &p) {
  // no params -> MODEV
  if (Channel::is_valid_channel_name(_channelName)) {
    std::map<std::string, Channel *>::iterator it =
        server.channels.find(_channelName);
...
void Mode::_mode_with_noparams(Channel *channel, std::string &currentMode,
...
void Mode::_mode_k(Channel *channel, Parser &p, std::string &currentMode,
                   std::string &currentParam, std::string &validPositiveModes,
                   std::string &validModesParams) {
  size_t invalidChar = 0;
  if (!currentParam.empty())
    invalidChar = std::count_if(currentParam.begin(), currentParam.end(),
                                Utils::is_invalid_char_key);
...
void Mode::_mode_l(Channel *channel, Parser &p, std::string &currentMode,
                   std::string &currentParam, std::string &validPositiveModes,
                   std::string &validModesParams) {
  size_t invalidChar = 0;
  if (!currentParam.empty())
...
void Mode::_mode_o(Server &server, Channel *channel, Parser &p,
                   std::string &currentMode, std::string &currentParam,
                   std::string &validPositiveModes,
                   std::string &validNegativeModes,
                   std::string &validModesParams) {
  size_t invalidChar = 0;
  invalidChar = std::count_if(currentParam.begin(), currentParam.end(),
                              Utils::is_invalid_char_nick);
...
void Mode::execute(Server &server, Client &client) {
  ...
  while (!_modeQueue.empty()) {
    ...
    if (!Utils::is_char_of(currentMode[1],
                           std::string(VALID_CHAN_MODE_NOPARAM)) &&
        !Utils::is_char_of(currentMode[1],
                           std::string(VALID_CHAN_MODE_PARAM))) {
      p.response(ERR_UNKNOWNMODE, currentMode.substr(1));
      continue;
    }
    // mode need params but there isn't
    if ((currentMode == "+k" || currentMode == "+l" || currentMode[1] == 'o') &&
        _paramsQueue.empty()) {
      p.response(ERR_NEEDMOREPARAMS, "MODE");
      continue;
    }
    ...
    if (Utils::is_char_of(currentMode[1],
                          std::string(VALID_CHAN_MODE_NOPARAM))) {
      _mode_with_noparams(channel, currentMode, validPositiveModes,
                          validNegativeModes);
      continue;
    }
    ...
    if (Utils::is_char_of(currentMode[1], std::string(VALID_CHAN_MODE_PARAM))) {
      ...
      if (currentMode[0] == '-' &&
          (currentMode[1] == 'k' || currentMode[1] == 'l')) {
      ...
      if (currentMode[1] == 'k') {
        _mode_k(channel, p, currentMode, currentParam, validPositiveModes,
                validModesParams);
      } else if (currentMode[1] == 'l') {
        _mode_l(channel, p, currentMode, currentParam, validPositiveModes,
                validModesParams);
      } else if (currentMode[1] == 'o') {
        _mode_o(server, channel, p, currentMode, currentParam,
                validPositiveModes, validNegativeModes, validModesParams);
      }
    }
  }
