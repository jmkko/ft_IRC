...
{
	Channel* channel = p.get_server()->find_channel_by_name(chanName);
	if (!channel) {
	...
	for (size_t i = 0; i < usersNames.size(); i++) {
        ...
        if (!channel->is_operator(*p.get_client())) {
            ...
            channel->broadcast(*p.get_server(), TRANSFER_KICK, channel->get_name() + " " + target->get_nickname(), p.get_client(), _msg);
