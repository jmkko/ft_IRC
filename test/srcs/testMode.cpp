...
void mode_pluso_should_grant_op_and_kick(Server& s)
{
    try {
		...
		std::string reply = recv_lines(sop);
        AssertReply ar(reply);
        ar.contains("MODE").contains("+o"); // members don't receive broadcast TOFIX
        // reply = recv_lines(sop2);
        // ar.handle_new_reply(reply);
        // ar.contains("MODE").contains("+o"); // sop2 does not receive broadcast msg TOFIX
		// test 2
		send_line(sop2, validKickMsg);
		reply  = recv_lines(so);
		LOG_TEST.debug("reply to roro after KICK", reply);
		ar.handle_new_reply(reply);
		ar.contains("KICK");
